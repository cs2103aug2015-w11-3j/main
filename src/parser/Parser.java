...
public class Parser implements ParserInterface {
    ...
    private static final String REGEX_UNVALIDATED_DATE = ".+"; // still
                                                               // needs to
                                                               // get parsed
                                                               // by date
                                                               // formatter
    ...
    private static final String REGEX_ADD_FLT = (new StringBuilder()).append('^').append(REGEX_GRP_NAME).append('$')
            .toString();

    // <name>; <start field identifier> <start>
    private final Pattern P_ADD_START;
    private static final String REGEX_ADD_START = (new StringBuilder()).append('^').append(REGEX_GRP_NAME)
            .append(REGEX_NAME_DATE_DELIM).append(REGEX_MATCHING_START_FIELD_ALIAS).append(REGEX_WHITESPACE)
            .append(REGEX_GRP_START).append('$').toString();

    // <name>; <end field identifier> <end>
    private final Pattern P_ADD_END;
    private static final String REGEX_ADD_END = (new StringBuilder()).append('^').append(REGEX_GRP_NAME)
            .append(REGEX_NAME_DATE_DELIM).append(REGEX_MATCHING_END_FIELD_ALIAS).append(REGEX_WHITESPACE)
            .append(REGEX_GRP_END).append('$').toString();

    // <name>; <start field identifier> <start> <end field identifier> <end>
    private final Pattern P_ADD_EVT;
    private static final String REGEX_ADD_EVT = (new StringBuilder()).append('^').append(REGEX_GRP_NAME)
            .append(REGEX_NAME_DATE_DELIM).append(REGEX_MATCHING_START_FIELD_ALIAS).append(REGEX_WHITESPACE)
            .append(REGEX_GRP_START).append(REGEX_WHITESPACE).append(REGEX_MATCHING_END_FIELD_ALIAS)
            .append(REGEX_WHITESPACE).append(REGEX_GRP_END).append('$').toString();
    ...
    private static final String REGEX_UPD_NAME = (new StringBuilder()).append('^').append(REGEX_GRP_UID)
            .append(REGEX_WHITESPACE).append(REGEX_MATCHING_NAME_FIELD_ALIAS).append(REGEX_WHITESPACE)
            .append(REGEX_GRP_NEW_NAME).append('$').toString();
    private final Pattern P_UPD_START;
    private static final String REGEX_UPD_START = (new StringBuilder()).append('^').append(REGEX_GRP_UID)
            .append(REGEX_WHITESPACE).append(REGEX_MATCHING_START_FIELD_ALIAS).append(REGEX_WHITESPACE)
            .append(REGEX_GRP_NEW_DATE).append('$').toString();
    private final Pattern P_UPD_END;
    private static final String REGEX_UPD_END = (new StringBuilder()).append('^').append(REGEX_GRP_UID)
            .append(REGEX_WHITESPACE).append(REGEX_MATCHING_END_FIELD_ALIAS).append(REGEX_WHITESPACE)
            .append(REGEX_GRP_NEW_DATE).append('$').toString();
    ...
    private static final String REGEX_UID_ONLY = (new StringBuilder()).append('^').append(REGEX_GRP_UID).append('$')
            .toString();
    ...
    private static final String REGEX_GRP_FIL_LO_BOUND = regexNamedGrp(REGEX_UNVALIDATED_DATE, GRPNAME_FIL_LO_BOUND);
    private static final String REGEX_GRP_FIL_HI_BOUND = regexNamedGrp(REGEX_UNVALIDATED_DATE, GRPNAME_FIL_HI_BOUND);

    // before|bef <key(date)>
    private final Pattern P_FILTER_BEF;
    private static final String REGEX_FILTER_BEF = (new StringBuilder()).append('^')
            .append(regexContaining(Aliases.FILTER_ARG_BEF)).append(REGEX_WHITESPACE).append(REGEX_GRP_FIL_HI_BOUND)
            .append('$').toString();
            // "^(?:before|bef)\\s+(?<key>.+)$";

    // after|aft <key(date)>
    private final Pattern P_FILTER_AFT;
    private static final String REGEX_FILTER_AFT = (new StringBuilder()).append('^')
            .append(regexContaining(Aliases.FILTER_ARG_AFT)).append(REGEX_WHITESPACE).append(REGEX_GRP_FIL_LO_BOUND)
            .append('$').toString();
            // "^(?:after|aft)\\s+(?<key>.+)$";

    // between|b/w|btw|from|start <key1(date)> and|to|till|until|end
    // <key2(date)>
    private final Pattern P_FILTER_BTW;
    private static final String REGEX_FILTER_BTW = (new StringBuilder()).append('^')
            .append(regexContaining(Aliases.FILTER_ARG_BTW_START)).append(REGEX_WHITESPACE)
            .append(REGEX_GRP_FIL_LO_BOUND).append(REGEX_WHITESPACE).append(regexContaining(Aliases.FILTER_ARG_BTW_END))
            .append(REGEX_WHITESPACE).append(REGEX_GRP_FIL_HI_BOUND).append('$').toString();
    ...
    private static final String regexContaining(String[] tokens) {
        ...
        for (String tok : tokens) {
            sb.append("\\Q").append(tok).append("\\E"); // necesary
                                                        // escaping
            sb.append('|');
        }
        if (tokens.length > 0) {
    ...
    public Command parseCommand(String rawInput) {
        ...
        // Splits input string at first whitespace substring, trimming
        // trailing whitespace
        String[] cmdAndArgs = P_WHITESPACE.split(rawInput.trim(), 2);
        // if no command args, set the args half of the array to empty
        // string
    ...
    private Command parseFilterDate(String args) {
        ...
        if (m.matches()) {
            try {
                max = parseDate(m.group(GRPNAME_FIL_HI_BOUND), false);
                min = Utilities.absBeginningTime();// new
                                                   // Date(0);
                return makeFilterDate(min, max);
            } catch (ParseException pe) {
                ;
        ...
        if (m.matches()) {
            try {
                min = parseDate(m.group(GRPNAME_FIL_LO_BOUND), true);
                max = Utilities.absEndingTime();// new
                                                // Date(Long.MAX_VALUE);
                return makeFilterDate(min, max);
            } catch (ParseException pe) {
                ;
     ...
     * assert(token != null); token = token.toLowerCase(); if
     * (arrayContains(Aliases.FIELD_NAME, token)) { return Task.DataType.NAME; }
     * if (arrayContains(Aliases.FIELD_START_DATE, token)) { return
     * Task.DataType.DATE_START; } if (arrayContains(Aliases.FIELD_END_DATE,
     * token)) { return Task.DataType.DATE_END; } throw new ParseException("",
     * -1); }
     */
    /*
     * Object parseFieldValue (Task.DataType key, String valStr) throws
     * ParseException, IllegalArgumentException { assert(key != null && valStr
     * != null); switch (key) { case NAME : return parseText(valStr); case
     * DATE_START : // Fallthrough case DATE_END : return parseDate(valStr);
     * default : throw new IllegalArgumentException(
     * "key must be amongst : NAME, DATE_START, DATE_END"); } }
     */
